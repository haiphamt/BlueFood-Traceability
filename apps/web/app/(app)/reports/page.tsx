import { createSupabaseServerClient } from '@/lib/supabase/server';
import { requireRole } from '@/lib/auth';
import { BATCH_STATUSES, BATCH_STATUS_LABELS } from '@bluefood/shared';
import { ReportsPeriodBar } from '@/components/reports-period-bar';
import { formatDate } from '@/lib/utils';
import { Package, Truck, QrCode, ShieldAlert, Download, Home, ChevronRight } from 'lucide-react';
import Link from 'next/link';
import type { BatchStatus } from '@bluefood/shared';
import type { LucideIcon } from 'lucide-react';

function MiniBarChart({ bars, color }: { bars: number[]; color: string }) {
  return (
    <div className="flex items-end gap-[3px] h-10 w-16 flex-shrink-0">
      {bars.map((h, i) => (
        <div
          key={i}
          style={{
            height: `${h}%`,
            backgroundColor: color,
            opacity: 0.25 + (i / (bars.length - 1)) * 0.75,
          }}
          className="flex-1 rounded-[2px] min-h-[3px]"
        />
      ))}
    </div>
  );
}

interface PageProps {
  searchParams: Promise<{ period?: string }>;
}

interface KpiItem {
  label: string;
  value: number;
  color: string;
  bg: string;
  icon: LucideIcon;
  href: string;
  footer: string;
  bars: number[];
  borderHover: string;
}

function periodToRange(period?: string): { gte?: string; lt?: string } {
  if (period === '7d') return { gte: new Date(Date.now() - 7 * 86_400_000).toISOString() };
  if (period === '30d') return { gte: new Date(Date.now() - 30 * 86_400_000).toISOString() };
  if (period === '2026') return { gte: '2026-01-01T00:00:00.000Z', lt: '2027-01-01T00:00:00.000Z' };
  return {};
}

const PERIOD_LABEL: Record<string, string> = {
  '7d':   '7 ngày qua',
  '30d':  '30 ngày qua',
  '2026': 'Năm 2026',
  '':     'Tất cả thời gian',
};

const STATUS_BAR_COLOR: Partial<Record<BatchStatus, string>> = {
  draft:             '#4a5568',
  created:           '#adc6ff',
  harvested:         '#22c55e',
  packed:            '#22c55e',
  quality_checked:   '#22c55e',
  in_transit:        '#ddb7ff',
  received_at_store: '#22c55e',
  sold:              '#22c55e',
  recalled:          '#ffb4ab',
  cancelled:         '#737373',
};

const ACTIVE_STATUSES = new Set<BatchStatus>([
  'created', 'harvested', 'packed', 'quality_checked', 'in_transit', 'received_at_store',
]);

function shipmentCode(id: string) {
  return 'TRK-' + id.replace(/-/g, '').slice(0, 4).toUpperCase();
}

export default async function ReportsPage({ searchParams }: PageProps) {
  await requireRole(['admin']);

  const { period } = await searchParams;
  const { gte, lt } = periodToRange(period);
  const now = new Date();
  const today = now.toISOString().split('T')[0];
  const in30Days = new Date(Date.now() + 30 * 86_400_000).toISOString().split('T')[0];

  const supabase = await createSupabaseServerClient();

  const [batchesRes, qrRes, certsRes, lateShipmentsRes] = await Promise.all([
    (() => {
      let q = supabase
        .from('batches')
        .select('id, status, product_id, supplier_id, products(name), suppliers(name)');
      if (gte) q = q.gte('created_at', gte);
      if (lt)  q = q.lt('created_at', lt);
      return q;
    })(),
    (() => {
      let q = supabase.from('qr_scan_logs').select('id', { count: 'exact', head: true });
      if (gte) q = q.gte('scanned_at', gte);
      if (lt)  q = q.lt('scanned_at', lt);
      return q;
    })(),
    supabase
      .from('certificates')
      .select('id, certificate_type, issuer, expires_at, batches(batch_code, suppliers(name))')
      .lte('expires_at', in30Days)
      .gte('expires_at', today)
      .order('expires_at')
      .limit(10),
    supabase
      .from('shipments')
      .select('id, vehicle_code, planned_arrival_at, from_location, to_location, batches(batch_code)')
      .lt('planned_arrival_at', now.toISOString())
      .neq('status', 'delivered')
      .order('planned_arrival_at')
      .limit(10),
  ]);

  const batches = batchesRes.data ?? [];
  const total = batches.length;

  const statusCounts: Record<string, number> = {};
  for (const b of batches) {
    statusCounts[b.status] = (statusCounts[b.status] ?? 0) + 1;
  }

  const activeCount    = batches.filter(b => ACTIVE_STATUSES.has(b.status as BatchStatus)).length;
  const qrCount        = qrRes.count ?? 0;
  const certsExpiring  = (certsRes.data ?? []).length;
  const lateCount      = (lateShipmentsRes.data ?? []).length;

  const productMap: Record<string, { name: string; count: number }> = {};
  for (const b of batches) {
    if (!b.product_id) continue;
    const name = (b.products as any)?.name ?? 'Không rõ';
    if (!productMap[b.product_id]) productMap[b.product_id] = { name, count: 0 };
    productMap[b.product_id].count++;
  }
  const top5Products = Object.values(productMap).sort((a, b) => b.count - a.count).slice(0, 5);
  const maxProduct = top5Products[0]?.count || 1;

  const supplierMap: Record<string, { name: string; count: number }> = {};
  for (const b of batches) {
    if (!b.supplier_id) continue;
    const name = (b.suppliers as any)?.name ?? 'Không rõ';
    if (!supplierMap[b.supplier_id]) supplierMap[b.supplier_id] = { name, count: 0 };
    supplierMap[b.supplier_id].count++;
  }
  const top5Suppliers = Object.values(supplierMap).sort((a, b) => b.count - a.count).slice(0, 5);
  const maxSupplier = top5Suppliers[0]?.count || 1;

  const periodLabel = PERIOD_LABEL[period ?? ''] ?? 'Tất cả thời gian';
  const exportHref  = `/api/reports/export${period ? `?period=${encodeURIComponent(period)}` : ''}`;

  const kpiItems: KpiItem[] = [
    { label: 'Tổng lô hàng',          value: total,         color: '#adc6ff', bg: 'rgba(173,198,255,0.10)', icon: Package,     href: '/batches',      footer: 'lô trong kỳ báo cáo', bars: [40,60,50,80,70,90,60,80], borderHover: 'hover:border-blue-200   dark:hover:border-[rgba(173,198,255,0.3)]' },
    { label: 'Lô đang hoạt động',      value: activeCount,   color: '#22c55e', bg: 'rgba(34,197,94,0.10)',  icon: Package,     href: '/batches',      footer: 'đang trong chuỗi', bars: [30,50,70,60,80,70,90,80], borderHover: 'hover:border-emerald-200 dark:hover:border-[rgba(34,197,94,0.3)]' },
    { label: 'Lượt quét QR',           value: qrCount,       color: '#fb923c', bg: 'rgba(251,146,60,0.10)', icon: QrCode,      href: '/reports',      footer: 'lượt quét ghi nhận', bars: [50,40,60,50,70,60,80,70], borderHover: 'hover:border-orange-200  dark:hover:border-[rgba(251,146,60,0.3)]' },
    { label: 'Chứng chỉ sắp hết hạn', value: certsExpiring, color: '#ffb77a', bg: 'rgba(255,183,122,0.10)', icon: ShieldAlert, href: '/certificates', footer: 'cần kiểm tra sớm', bars: [60,50,40,60,50,70,60,50], borderHover: 'hover:border-amber-200   dark:hover:border-[rgba(255,183,122,0.3)]' },
  ];

  return (
    <div className="max-w-[1400px] mx-auto space-y-5">

      {/* Breadcrumb + header */}
      <div>
        <div className="admin-breadcrumb">
          <Home size={13} />
          <span>/</span>
          <span className="admin-breadcrumb-current">Báo cáo</span>
        </div>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="admin-page-title">
              Báo cáo hệ thống
            </h1>
            <p className="admin-muted-strong text-sm mt-0.5">
              {periodLabel}
            </p>
          </div>
          <a
            href={exportHref}
            className="admin-primary-button flex-shrink-0"
          >
            <Download size={14} />
            Xuất báo cáo CSV
          </a>
        </div>
      </div>

      {/* Period filter */}
      <ReportsPeriodBar />

      {/* KPI cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiItems.map(({ label, value, color, bg, icon: Icon, href, footer, bars, borderHover }) => (
          <Link
            key={label}
            href={href}
            className={[
              'group rounded-2xl border p-5 flex flex-col gap-3',
              'transition-all duration-200 hover:-translate-y-0.5',
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22c55e] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#0d0d0f]',
              'bg-panel dark:bg-[#171717] border-line dark:border-[#2a2a2d]',
              borderHover,
            ].join(' ')}
          >
            <div className="flex items-center gap-2.5">
              <div
                className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center transition-transform duration-200 group-hover:scale-110"
                style={{ backgroundColor: bg }}
              >
                <Icon size={17} style={{ color }} strokeWidth={1.8} />
              </div>
              <span className="text-[12px] font-medium admin-muted leading-snug">{label}</span>
            </div>

            <div className="flex items-end justify-between gap-2">
              <p className="text-4xl font-black leading-none" style={{ color }}>
                {value.toLocaleString('vi-VN')}
              </p>
              <MiniBarChart bars={bars} color={color} />
            </div>

            <div className="flex items-center justify-between">
              <p className="text-[11px] admin-muted">{footer}</p>
              <ChevronRight size={12} className="admin-muted flex-shrink-0 opacity-0 -translate-x-1 group-hover:opacity-60 group-hover:translate-x-0 transition-all duration-200" />
            </div>
          </Link>
        ))}
      </div>

      {/* 3-col analytics */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">

        {/* Batch status bars */}
        <div className="admin-card p-5 transition-colors duration-200 hover:border-slate-200 dark:hover:border-[#3a3a3d]">
          <h2 className="admin-ink text-sm font-semibold mb-4">Lô hàng theo trạng thái</h2>
          <div className="space-y-3">
            {BATCH_STATUSES.map((s) => {
              const cnt = statusCounts[s] ?? 0;
              const pct = total > 0 ? Math.round((cnt / total) * 100) : 0;
              const barColor = STATUS_BAR_COLOR[s] ?? '#737373';
              return (
                <div key={s}>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="admin-muted text-[11px] truncate pr-2">
                      {BATCH_STATUS_LABELS[s]}
                    </span>
                    <span className="admin-ink text-[11px] font-semibold flex-shrink-0">
                      {cnt} <span className="admin-muted-strong">({pct}%)</span>
                    </span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden bg-[var(--color-surface-2)] dark:bg-[#1f1f22]">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${pct}%`, backgroundColor: barColor }}
                    />
                  </div>
                </div>
              );
            })}
          </div>
        </div>

        {/* Top 5 products */}
        <div className="admin-card p-5 transition-colors duration-200 hover:border-slate-200 dark:hover:border-[#3a3a3d]">
          <h2 className="admin-ink text-sm font-semibold mb-4">Top 5 sản phẩm</h2>
          {top5Products.length === 0 ? (
            <p className="admin-muted-strong text-xs text-center py-4">Chưa có dữ liệu</p>
          ) : (
            <div className="space-y-3.5">
              {top5Products.map(({ name, count }, i) => (
                <div key={name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="admin-muted-strong text-[10px] font-bold w-4 flex-shrink-0 text-right">{i + 1}.</span>
                      <span className="admin-muted text-[11px] truncate">{name}</span>
                    </div>
                    <span className="text-[11px] font-bold flex-shrink-0 ml-2" style={{ color: '#22c55e' }}>{count}</span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden bg-[var(--color-surface-2)] dark:bg-[#1f1f22]">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${Math.round((count / maxProduct) * 100)}%`, backgroundColor: '#22c55e' }}
                    />
                  </div>
                </div>
              ))}
            </div>
          )}
        </div>

        {/* Top 5 suppliers */}
        <div className="admin-card p-5 transition-colors duration-200 hover:border-slate-200 dark:hover:border-[#3a3a3d]">
          <h2 className="admin-ink text-sm font-semibold mb-4">Top 5 nhà cung cấp</h2>
          {top5Suppliers.length === 0 ? (
            <p className="admin-muted-strong text-xs text-center py-4">Chưa có dữ liệu</p>
          ) : (
            <div className="space-y-3.5">
              {top5Suppliers.map(({ name, count }, i) => (
                <div key={name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="admin-muted-strong text-[10px] font-bold w-4 flex-shrink-0 text-right">{i + 1}.</span>
                      <span className="admin-muted text-[11px] truncate">{name}</span>
                    </div>
                    <span className="text-[11px] font-bold flex-shrink-0 ml-2" style={{ color: '#ddb7ff' }}>{count}</span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden bg-[var(--color-surface-2)] dark:bg-[#1f1f22]">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${Math.round((count / maxSupplier) * 100)}%`, backgroundColor: '#ddb7ff' }}
                    />
                  </div>
                </div>
              ))}
            </div>
          )}
        </div>
      </div>

      {/* Operational alert tables */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

        {/* Certificates expiring */}
        <div className="admin-card">
          <div className="admin-card-toolbar px-4 py-3 flex items-center justify-between">
            <h2 className="admin-ink text-[13px] font-semibold flex items-center gap-2">
              <ShieldAlert size={14} style={{ color: '#ffb77a' }} />
              Chứng chỉ sắp hết hạn (30 ngày)
            </h2>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(255,183,122,0.12)', color: '#ffb77a' }}>
              {certsExpiring}
            </span>
          </div>
          {certsRes.data && certsRes.data.length > 0 ? (
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-line dark:border-[#2a2a2d]">
                  <th className="admin-th px-4 py-2 text-[10px]">Loại chứng chỉ</th>
                  <th className="admin-th px-4 py-2 text-[10px]">Mã lô / NCC</th>
                  <th className="admin-th px-4 py-2 text-[10px] text-right">Hết hạn</th>
                </tr>
              </thead>
              <tbody>
                {certsRes.data.map((c: any) => {
                  const daysLeft = Math.ceil((new Date(c.expires_at).getTime() - now.getTime()) / 86_400_000);
                  return (
                    <tr key={c.id} className="admin-row">
                      <td className="admin-ink px-4 py-2.5 text-[12px] font-medium">{c.certificate_type}</td>
                      <td className="px-4 py-2.5">
                        <div className="text-[11px] font-medium" style={{ color: '#22c55e' }}>{c.batches?.batch_code ?? '—'}</div>
                        <div className="admin-muted-strong text-[10px]">{(c.batches?.suppliers as any)?.name ?? '—'}</div>
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        <div className="text-[11px] font-semibold" style={{ color: '#ffb77a' }}>{formatDate(c.expires_at)}</div>
                        <div className="admin-muted-strong text-[10px]">còn {daysLeft} ngày</div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
          ) : (
            <p className="admin-muted-strong px-4 py-8 text-center text-sm">Không có chứng chỉ sắp hết hạn</p>
          )}
        </div>

        {/* Late shipments */}
        <div className="admin-card">
          <div className="admin-card-toolbar px-4 py-3 flex items-center justify-between">
            <h2 className="admin-ink text-[13px] font-semibold flex items-center gap-2">
              <Truck size={14} style={{ color: '#ffb77a' }} />
              Vận chuyển trễ ETA
            </h2>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(255,183,122,0.12)', color: '#ffb77a' }}>
              {lateCount}
            </span>
          </div>
          {lateShipmentsRes.data && lateShipmentsRes.data.length > 0 ? (
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-line dark:border-[#2a2a2d]">
                  <th className="admin-th px-4 py-2 text-[10px]">Mã chuyến</th>
                  <th className="admin-th px-4 py-2 text-[10px]">Mã lô / Lộ trình</th>
                  <th className="admin-th px-4 py-2 text-[10px] text-right">ETA dự kiến</th>
                </tr>
              </thead>
              <tbody>
                {lateShipmentsRes.data.map((s: any) => {
                  const overdueDays = Math.ceil((now.getTime() - new Date(s.planned_arrival_at).getTime()) / 86_400_000);
                  return (
                    <tr key={s.id} className="admin-row">
                      <td className="px-4 py-2.5">
                        <div className="admin-ink text-[12px] font-bold">{shipmentCode(s.id)}</div>
                        <div className="admin-muted-strong text-[10px]">{s.vehicle_code ?? '—'}</div>
                      </td>
                      <td className="px-4 py-2.5">
                        <div className="text-[11px] font-medium" style={{ color: '#22c55e' }}>{(s.batches as any)?.batch_code ?? '—'}</div>
                        <div className="admin-muted-strong text-[10px] truncate max-w-[160px]">
                          {s.from_location ?? '—'} → {s.to_location ?? '—'}
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        <div className="text-[11px] font-semibold" style={{ color: '#ffb77a' }}>{formatDate(s.planned_arrival_at)}</div>
                        <div className="text-[10px]" style={{ color: '#ffb4ab' }}>trễ {overdueDays} ngày</div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
          ) : (
            <p className="admin-muted-strong px-4 py-8 text-center text-sm">Không có chuyến hàng trễ ETA</p>
          )}
        </div>
      </div>
    </div>
  );
}
